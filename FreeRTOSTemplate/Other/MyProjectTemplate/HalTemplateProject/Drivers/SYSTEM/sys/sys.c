...
//static uint8_t sys_stm32_clock_init(uint32_t plln, uint32_t pllm, uint32_t pllp, uint32_t pllq);   //配置系统时钟
void SystemClock_Config(void);

//全局
static void sys_nvic_set_vector_table(uint32_t baseaddr, uint32_t offset);     //设置中断偏移量
static void sys_standby(void);             // 进入待机模式 
static void sys_soft_reset(void);          // 系统软复位 
